...
namespace vog
{
	EditorLayer::EditorLayer(const std::string& name_)
		:
		Layer(name_), m_cameraController()
	{
		...
		auto& m_ecs = m_pActiveScene->getECS();
	}
	...
	void EditorLayer::onAttach()
	{
		...
		auto sphere = m_scenePanel.createSphere();
		auto plane = m_scenePanel.createPlane();
		
		plane.getComponent<TransformComponent>().scale = { 5.0f, 5.0f, 5.0f };

		auto& sphere_transform = sphere.getComponent<TransformComponent>();
		sphere_transform.translation.y = 4.0f;


		class MoveEntity : public NativeScriptEntity
		{
		public:
			MoveEntity() = default;
			virtual ~MoveEntity() = default;
			...
			{
				VOG_CORE_LOG_INFO("Move Entity: onDestroy!");
			};

			virtual void onUpdate(float dt_)
			{
				//VOG_CORE_LOG_INFO("Move Entity: onUpdate!");

				auto& translation = getComponent<TransformComponent>().translation;
				float speed = 5.0f;

				if (Input::isKeyPressed(KeyCode::A))
					translation.x -= speed * dt_;
				if (Input::isKeyPressed(KeyCode::D))
					translation.x += speed * dt_;
				if (Input::isKeyPressed(KeyCode::W))
					translation.y += speed * dt_;
			...
			{
				//VOG_CORE_LOG_INFO("Move Entity: onTriggerEnter!");
			};
		};
		m_moveEntity = m_scenePanel.createSphere({}, "move_entity");
		m_moveEntity.addComponent<NativeScriptComponent>().bind<MoveEntity>();
	...
	void EditorLayer::onUpdate(float timestep_)
	{
		// Resize
	...
	void EditorLayer::onEvent(Event& event_)
	{
...
#pragma endregion
	...
	void EditorLayer::renderColliderOutline()
	{
	...
	void EditorLayer::renderLightOutline()
	{
		auto& transform_cm		= m_pActiveScene->getComponentManager<TransformComponent>();
	...
	Vector2f EditorLayer::getViewportSize()
	{
	...
	Vector2f EditorLayer::getViewportMousePosition()
	{
		auto viewportMinRegion = ImGui::GetWindowContentRegionMin();
		auto viewportMaxRegion = ImGui::GetWindowContentRegionMax();
		auto viewportOffset = ImGui::GetWindowPos();
	...
	bool EditorLayer::onMouseButtonPressed(MouseButtonPressedEvent& event_)
	{
	...
	bool EditorLayer::onKeyPressed(KeyPressedEvent& event_)
	{
