...
vec4 phongLight(vec3 ws_pos_, vec3 normal_, vec4 albedo_)
{
    ...
    for(int i = 0; i < u_current_light_count; ++i)
    {
        vec3 lightColor 		= u_light_colors[i].rgb;
        
        float lightIntensity 	= u_light_params[i].x;

        vec3 lightPos       	= u_light_positions[i].xyz;
        //vec3 lightDir       	= u_light_colors[i].xyz;
        float isDirectional  	= 1 - u_light_positions[i].w;

        //float isSpotlight         = u_light_params[i].x;
        float lightSpotAngle      = u_light_params[i].y;
        float lightInnerSpotAngle = u_light_params[i].z;
        float lightRange          = u_light_params[i].w;

        vec3 toLight = lightPos - ws_pos_;
        //vec3 L = mix(toLight, lightDir, isDirectional);
        vec3 L = toLight;

        float lengthSq = dot(toLight, toLight); //* (1 - isDirectional);
        ...
        float specularIntensity = mix(0, 0.4, diffuseIntensity);
        ...
        float attenuation = 1.0 - clamp(lengthSq / (lightRange * lightRange), 0.0, 1.0);
...
void main()
{
    ...
    o_color = phongLight(ws_pos, normal, albedo);
    ...
    o_brightColor = vec4(mix(vec3(0.0), o_color.rgb, brightness), 1.0);
}